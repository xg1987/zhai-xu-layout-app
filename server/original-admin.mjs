...
export async function originalAdmin(c){
 ...
 if(path==='/api/account/settings'&&method==='GET')return json({user:publicUser(user),software:{name:'家居风水',version:'0.3.2',language:'简体中文'}});
 if(!path.startsWith('/api/admin/'))return null;
 if(user.role!=='admin')fail('此账号没有管理员权限',403);
 if(path==='/api/admin/access'&&method==='GET')return json({user:publicUser(user)});
 if(path==='/api/admin/settings'&&method==='GET')return json({user:publicUser(user),software:{name:'家居风水',version:'0.3.2',language:'简体中文'}});
