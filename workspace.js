...
function setState(next){state=next;$('upload').hidden=false;$('uploadButton').hidden=!!currentImage;$('uploadGuide').hidden=!!currentImage;$('fileAction').hidden=!currentImage;$('angleReadout').hidden=true;$('analyzeImage').hidden=!currentImage;if(currentImage){$('readout').textContent=angle.toFixed(1)+'°'}draw();renderPanel()}
