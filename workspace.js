...
function syncDesktopPlan(){const hasImage=!!currentImage;$('desktopPlanEmpty').hidden=hasImage;$('desktopPlanLoaded').hidden=!hasImage;if(hasImage){$('desktopPlanPreview').src=currentImage.url;$('desktopAnalyze').textContent=currentImage.savedPlan?.status==='complete'?'查看分析结果':'确认分析家居图'}else $('desktopPlanPreview').removeAttribute('src')}
function setState(next){state=next;$('upload').hidden=false;$('uploadButton').hidden=!!currentImage;$('fileAction').hidden=!currentImage;$('angleReadout').hidden=true;$('analyzeImage').hidden=!currentImage;if(currentImage){$('readout').textContent=angle.toFixed(1)+'°'}syncDesktopPlan();draw();renderPanel()}
...
['work','history'].forEach(id=>$(id).onclick=()=>navigate(id));$('uploadButton').onclick=chooseImage;$('desktopUpload').onclick=chooseImage;$('desktopAnalyze').onclick=()=>requestAnalysis();$('desktopReplace').onclick=chooseImage;$('desktopDelete').onclick=deleteImage;$('replaceImage').onclick=chooseImage;$('deleteImage').onclick=deleteImage;$('imageInput').onchange=e=>loadImage(e.target.files[0]);$('zoomIn').onclick=()=>zoom(.1);$('zoomOut').onclick=()=>zoom(-.1);$('zoom').onclick=fitWindow;$('fitWindow').onclick=fitWindow;
