...
const register=location.pathname.startsWith('/register'),admin=new URLSearchParams(location.search).get('admin')==='1';
...
document.title=(register?'注册账号':admin?'管理员登录':'登录')+' · 家居风水';
document.querySelector('#authTitle').textContent=register?'注册账号':admin?'管理员登录':'登录';
...
document.querySelector('#switchAuth').textContent=register?'已有账号？立即登录':admin?'用户登录':'注册账号';
document.querySelector('#switchAuth').href=register||admin?'/login':'/register';
document.querySelector('#adminEntry').hidden=admin||register;
...
form.onsubmit=async event=>{
 ...
 delete body.confirm;body.audience=admin?'admin':'user';submit.disabled=true;
 try{
