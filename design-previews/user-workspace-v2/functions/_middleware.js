...
export async function onRequest(context){
 ...
 if(['/', '/index.html','/phone','/phone.html','/admin','/admin.html'].includes(path)){
  ...
  const next=await context.next();const response=new Response(next.body,next);response.headers.set('Cache-Control','no-store');return response;
 }
 return context.next();
}
